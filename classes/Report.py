...
class Report(object):
    ...
    def printToFile(self):
        ...
        text += "Your BMI is " + str(
            "%.1f" % user.getBMI()) + " and your weight status is " + user.getBMIStatus() + "\n\n"
        text += "Calories Report\n"
        text += "===========================================\n"
        text += "Daily calories you need to maintain your current weight is " + str(
            "%d" % user.getDailyCalories()) + "\n";
        diff = user.getWeightDiff();
        daily_calories_change = user.getCaloriesChange();
        if diff < 0:
            text += "Daily calories you need to lose weight (" + str("%.2f" % abs(diff)) + "kg/week) is " + str(
                "%d" % daily_calories_change) + "\n";
        elif diff > 0:
            text += "Daily calories you need to gain weight (" + str("%.2f" % abs(diff)) + "kg/week) is " + str(
                "%d" % daily_calories_change) + "\n";
        text += "\nSuggested meal\n"
        text += "===========================================\n"
